...
class EditEpisode extends Component {
    constructor(props){
        ...
        this.getSelectAnime = this.getSelectAnime.bind(this);
    ...
    addEpisodeSubmit(e){
        ...
        formData.append('id',this.state.dataEpisode.id);
        ...
        if (this.gambarInput.current.files[0]) {
            formData.append('gambar',this.state.gambar.raw);
        } else {
            formData.append('gambar',this.state.gambar.raw.name);
        }
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                if (this.state.dataEpisode.animeId === "") {
                    ...
                    return axios.put(uAPIlocal+'/api/v1/animeepisodelist',formData,config)
    ...
    async getSelectAnime() {
        try {
            await axios.get(uAPIlocal+'/api/v1/select/anime')
            .then(function(response) {
    ...
    async getEpisodeList() {
        try {
            await axios.get(uAPIlocal+'/api/v1/animeepisodelist/'+this.props.id)
            .then(function(response) {
                return response;
            })
            .then(response => {
                ...
                var selectedAnime = this.state.dataEpisode.animeOptions.find( ({ animeId }) => animeId === data.animeId );
                var selectedEpisode = { value: data.episode, label: data.episode };
                ...
                if (cek===0) {
                    ...
                    this.setState({
                        dataEpisode:{
                            ...
                            mkv_240p: data.mkv_240p,
                            mkv_360p: data.mkv_360p,
                            mkv_480p: data.mkv_480p,
                            mkv_720p: data.mkv_720p,
                            mkv_1080p: data.mkv_1080p,
                            mp4_240p: data.mp4_240p,
                            mp4_360p: data.mp4_360p,
                            mp4_480p: data.mp4_480p,
                            mp4_720p: data.mp4_720p,
                            mp4_1080p: data.mp4_1080p,
                        },
                        loading: false,
                        gambar: {
                            icon: false,
                            preview: uAPIlocal+'/'+data.gambar_url,
                            raw: {
                                name: data.gambar,
                            },
                        },
    ...
    componentDidMount(){
        ...
        // axios.get(uAPIlocal+'/api/v1/animeepisodelist/'+this.props.id)
        // .then(function(response) {
        //     return response;
        // })
        // .then(response => {
                    ...
                    <div className="container-fluid">
                        <div className="card">
                            <div className="card-header">
                                <h3 className="card-title">Edit Episode Anime</h3>
                            </div>
                                ...
                                <form onSubmit={e => this.addEpisodeSubmit(e)} className="form-horizontal" style={{padding:"10px"}}>
                                    ...
                                    <div className="form-group">
                                        <div className="col-sm-offset-2 col-sm-10">
                                            ...
                                            </button> <Link to="/admin/anime-episode" className="btn btn-warning" ><FontAwesomeIcon icon={faArrowLeft}/> Batal</Link>
                                        </div>
                                    </div>
